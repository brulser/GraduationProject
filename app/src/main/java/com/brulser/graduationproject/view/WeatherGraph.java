...
import com.brulser.graduationproject.util.DensityUtils;
...
public class WeatherGraph extends View {
    ...
    private void drawXValue(Canvas canvas) {
        ...
        for (GraphItem item : items) {
            xPoints.add(startX);
            //截取item字符串,進行上下拼接
            canvas.drawText(item.getxValue().substring(0,item.getxValue().indexOf(" ")), startX, verMargin / 2-8, paint);
            canvas.drawText(item.getxValue().substring(item.getxValue().indexOf(" ")+1), startX, verMargin / 2+17, paint);
            startX += period;
        }
    }
    ...
    private void drawGraphAxis(Canvas canvas) {
        ...
        paint.setStyle(Paint.Style.FILL);
        canvas.drawText("℃", width - hosMargin - 30, verUpMargin - 30, paint);
